...
int _write(int file, char *ptr, int len) {
    /* Sobrescreve a funcao padrao para direcionar o printf para a porta serial USART2 */
    HAL_UART_Transmit(&huart2, (uint8_t*)ptr, len, HAL_MAX_DELAY);
    return len;
}

float read_adc(void){
    /* Executa a aquisicao de dados do canal analogico e converte para ohms */
    ...
    {
        /* Armazena a conversao bruta de 12-bits do conversor analogico-digital */
...
float resistencia_media() {
    /* Filtro passa-baixa digital (media movel) para atenuar ruidos na leitura do sensor */
    static float buffer[BUFFER_SIZE] = {0};
    static int index = 0;
    static float soma = 0.0;
    static int count = 0;

    /* Atualiza a janela de amostragem removendo o dado antigo e inserindo o novo */
    soma -= buffer[index];
    buffer[index] = read_adc();
    soma += buffer[index];

    /* Atualiza o apontador do buffer circular */
    index = (index + 1) % BUFFER_SIZE;

    /* Tratamento para o regime transitorio inicial (preenchimento inicial do vetor) */
...
uint8_t resistencia_pwm(float res){
    /* Retorna o Duty Cycle correspondente a resistencia alvo consultando a matriz em memoria */

    if (res <= lut[0].resistance){ return lut[0].pwm; }
    if (res >= lut[LUT_SIZE-1].resistance){ return lut[LUT_SIZE-1].pwm; }

    for(int i = 0; i < LUT_SIZE-1; i++) {
        /* Itera sobre a Look-up Table para localizar a faixa de operacao atual */
        ...
        if(res >= r1 && res <= r2) {
            ...
            /* Calculo da interpolacao linear entre dois pontos conhecidos da curva do LDR */
...
void criar_lut(){
    /* Algoritmo de calibracao automatica do sistema fisico (LED acoplado ao LDR) */
    uint8_t index_lut = 0;
...
int main(void)
{
  ...
  /* Antiga chamada de interrupcao simples, substituida pelo DMA assincrono */
  ...
  while (1)
  {
    ...
    /* Formatacao do console com conversao explicita (casting) para inteiro */
...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t Size){
  ...
  /* Insere o terminador nulo para assegurar que os dados recebidos formem uma string valida */
  ...
  if (rx_dma_buffer[0] == 'R' || rx_dma_buffer[0] == 'r') {
    /* Faz o parsing da informacao recebida isolando apenas a grandeza escalar (int) */
    if (sscanf(rx_dma_buffer, "%*c %d", &nova_res_int) == 1) {
      /* Realiza o cast para float e solicita o ajuste no controle PWM do digipot */
      brilho_led = resistencia_pwm((float)nova_res_int);
      /* Feedback de execucao no console serial */
      printf("Resistencia alvo atualizada para: %d\r\n", nova_res_int);
    }
  }

  else if (rx_dma_buffer[0] == 'L' || rx_dma_buffer[0] == 'l') {
    /* Extrai os parametros formatados enviados pelo host para sobrescrever a memoria */
    if (sscanf(rx_dma_buffer, "%*c %d %d %d", &index, &novo_pwm, &nova_res_int) == 3) {
      if (index >= 0 && index < LUT_SIZE) {
        lut[index].pwm = (uint8_t)novo_pwm;
        lut[index].resistance = (float)nova_res_int; /* Conversao exigida pela definicao da struct */

        /* Confirmação ao usuario contendo as especificacoes que foram salvas */
  ...
  /* Rearma a recepcao em backgroud da UART atraves do canal de DMA configurado */
  HAL_UARTEx_ReceiveToIdle_DMA(&huart2, (uint8_t *)rx_dma_buffer, RX_BUFFER_SIZE);
  __HAL_DMA_DISABLE_IT(&hdma_usart2_rx, DMA_IT_HT);
}
}
